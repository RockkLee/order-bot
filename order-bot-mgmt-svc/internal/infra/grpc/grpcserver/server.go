...
func NewServer(orderSvc *ordersvc.Svc, db store.DB) *grpc.Server {
	server := grpc.NewServer()
	orderbotmgmtsvcpb.RegisterOrderSyncServiceServer(server, NewOrderSyncServer(orderSvc, db))
	return server
}

func NewListeningServer(addr string, orderSvc *ordersvc.Svc, db store.DB) (*grpc.Server, net.Listener, error) {
